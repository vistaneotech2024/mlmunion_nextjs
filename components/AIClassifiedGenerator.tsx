...
interface AIClassifiedGeneratorProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerated: (title: string, description: string, meta_description?: string, meta_keywords?: string, focus_keyword?: string) => void;
}

export function AIClassifiedGenerator({ isOpen, onClose, onGenerated }: AIClassifiedGeneratorProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setStep('generated');
      toast.success('SEO-optimized content generated successfully!');
    } catch (error: any) {
      ...
      toast.error(error?.message || 'Failed to generate description. Please check the console for details.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-medium text-gray-900">Generate Classified with AI</h3>
              </div>
              <button
                onClick={handleClose}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Join Our Health & Wellness Network Marketing Opportunity (or leave empty for AI-generated SEO title)"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Provide a brief description of the opportunity (minimum 50 characters). This will be used to generate a detailed 5-paragraph description."
                    rows={4}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                    disabled={loading}
                  />
              ...
              <div className="space-y-4">
                <div className="bg-green-50 border border-green-200 rounded-md p-4">
                  <p className="text-sm text-green-800 font-medium mb-2">
                    ✓ SEO-optimized content generated successfully!
                  </p>
                  <p className="text-xs text-green-700">
                    Review and edit if needed, then proceed to create your classified. The content includes proper HTML formatting, bold keywords, and SEO optimization.
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Generated Description (5 paragraphs, ~200 words each)
                  </label>
                  <div className="mt-1">
                    <RichTextEditor
                      value={generatedDescription}
                      onChange={(value) => setGeneratedDescription(value)}
                      className="min-h-[400px]"
                    />
                  </div>
                  <p className="mt-1 text-xs text-gray-500">
                    {generatedDescription ? generatedDescription.replace(/<[^>]*>/g, '').split(/\s+/).length : 0} words | {generatedDescription ? generatedDescription.replace(/<[^>]*>/g, '').length : 0} characters
                  </p>
                </div>
