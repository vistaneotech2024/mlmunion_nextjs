...
export function ClassifiedCategoryManagement() {
  const [categories, setCategories] = React.useState<ClassifiedCategory[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [showModal, setShowModal] = React.useState(false);
  const [editingCategory, setEditingCategory] = React.useState<ClassifiedCategory | null>(null);
  ...
  const toggleActive = async (category: ClassifiedCategory) => {
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-0">
        <h3 className="text-xl md:text-xl font-semibold text-gray-900">Classified Categories</h3>
        <button
          onClick={() => {
            setEditingCategory(null);
            setShowModal(true);
